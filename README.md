...
1. Nginx：TCP 四层负载均衡，将客户端连接均匀分发到多个 ChatServer 节点
2. ChatServer：业务处理节点，无状态设计，可水平扩展
3. Redis：
   - 发布订阅模式实现跨节点消息通信
   - 分布式缓存，存储在线用户路由信息
4. MySQL：持久化存储用户、好友、群组、离线消息等数据
5. 数据库连接池：避免频繁创建销毁数据库连接，提升高并发下的性能
...
friend 表（好友关系）
  ...
  字段名称    	类型  	说明   	约束                                      
  userid  	INT 	用户 ID	PRIMARY KEY, FOREIGN KEY REFERENCES user(id)
  friendid	INT 	好友 ID	PRIMARY KEY, FOREIGN KEY REFERENCES user(id)
...
group_member 表（群组成员）
  ...
  字段名称     	类型     	说明   	约束                                      
  groupid  	INT    	群组 ID	PRIMARY KEY, FOREIGN KEY REFERENCES chat_group(id)
  userid   	INT    	用户 ID	PRIMARY KEY, FOREIGN KEY REFERENCES user(id)
...
offline_message 表（离线消息）
  ...
  to_userid  	INT     	接收者用户 ID       	NOT NULL, FOREIGN KEY REFERENCES user(id)
  from_userid	INT     	发送者用户 ID       	NOT NULL, FOREIGN KEY REFERENCES user(id)
  groupid    	INT     	群组 ID（私聊为 NULL）	DEFAULT NULL, FOREIGN KEY REFERENCES chat_group(id)
...
运行
    ...
    enum EnMsgType {
        ...
        JOIN_GROUP_MSG_ACK= 10,  // 加入群组响应
        LEAVE_GROUP_MSG   = 11,  // 退出群组请求
        LEAVE_GROUP_MSG_ACK=12,  // 退出群组响应
        GROUP_CHAT_MSG    = 13,  // 群组聊天消息
        GROUP_INFO_MSG    = 14,  // 获取群组信息请求
        GROUP_INFO_MSG_ACK= 15,  // 获取群组信息响应
...
json
    ...
    {"msgid":2,"error":0,"id":13,"name":"zhang san","friends":[],"groups":[]}
...
- 修改配置文件 conf/mysql.conf 中的数据库连接信息
...
添加以下配置到 /etc/nginx/nginx.conf 的 http 块外部：
...
1. 退出登录后再次登录会显示 "被踢下线" 提示
2. 同一服务器节点登录时，离线消息可能无法正常加载
3. 客户端多线程输出存在乱序问题（cout 未加锁）
4. 客户端输入密码时明文显示
5. 好友 / 群组列表不会实时更新，需重新登录才能显示
6. 客户端没有注销账号功能
